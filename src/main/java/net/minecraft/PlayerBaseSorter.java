...
public final class PlayerBaseSorter implements Comparator<String> {
    ...
    public PlayerBaseSorter(List<String> list, Map<String, String[]> map, Map<String, String[]> map2, String string) {
        this.list = list;
    ...
    private void indexes(String string) {
        int var2 = (Integer)this.groupIndexes.get(string);
        Iterator var3 = ((Set)this.allInferiors.get(string)).iterator();
    ...
    private Set<String> build(String string, String string2) {
        Set var3 = (Set)this.allInferiors.get(string);
        if (var3 == null) {
            var3 = this.build(string, (Set)null, string2 != null ? string2 : string);
            if (var3 == null) {
                var3 = Empty;
            }

            this.allInferiors.put(string, var3);
        }
    ...
    private Set<String> build(String string, Set<String> set, String string2) {
        Set var4 = (Set)this.directInferiorsMap.get(string);
        if (var4 == null) {
            return (Set)set;
        } else {
            if (set == null) {
            ...
            while(var5.hasNext()) {
                ...
                if (var6 == string2) {
                    throw new UnsupportedOperationException("Can not sort PlayerBase classes for method '" + this.methodName + "'. Circular superiosity found including '" + string2 + "'");
                }
    ...
    private static Map<String, Set<String>> build(String string, Map<String, Set<String>> map, Map<String, Set<String>> map2, Map<String, Set<String>> map3, String[] strings) {
        if (map == null) {
            map = new Hashtable();
        }
        ...
        for(int var6 = 0; var6 < strings.length; ++var6) {
            if (strings[var6] != null) {
                var5.add(strings[var6]);
            }
        }

        if (map2 != null) {
            getOrCreateSet(map2, string).addAll(var5);
        }
        ...
        if (map3 != null) {
            ...
            while(var7.hasNext()) {
                getOrCreateSet(map3, (String)var7.next()).add(string);
            }
        }

        ((Map)map).put(string, var5);
        return (Map)map;
    }

    private static Set<String> getOrCreateSet(Map<String, Set<String>> map, String string) {
        return map.computeIfAbsent(string, k -> new HashSet());
    }

    public int compare(String string, String string2) {
        if (this.allInferiors != null) {
            if (((Set)this.allInferiors.get(string)).contains(string2)) {
                return 1;
            }

            if (((Set)this.allInferiors.get(string2)).contains(string)) {
                return -1;
            }
        }

        int var3 = ((Integer)this.groupIndexes.get(string)).compareTo((Integer)this.groupIndexes.get(string2));
        return var3 != 0 ? var3 : ((Integer)this.indexes.get(string)).compareTo((Integer)this.indexes.get(string2));
    }
}
