...
public class ForgeModCandidate implements ModCandidate {
    private final String id, destinationName;
    private Path path, destinationPath;
    private final ModDiscovererConfig config;

    public ForgeModCandidate(Path path, ModDiscovererConfig config) {
        this.path = path;
        this.destinationName = path.getFileName().toString();
        this.id = this.destinationName.replace(".jar", "").replace(".zip", "");
        this.config = config;
    }
